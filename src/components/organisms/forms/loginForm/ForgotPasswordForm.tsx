...
const ForgotPasswordForm: FC<IForgotPassword> = ({ verify }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      console.log(error);
      const message: string = error.response.data.message || error.response.data.errors[0]
      api.error({ message })
      setIsLoading(false)
    }
  }
    ...
    <>
      ...
      <form className='form bg-white d-flex flex-column rounded-5' onSubmit={handleSubmit} action="">
        ...
        <div className="d-flex flex-column align-items-center gap-3">
          <Button children='Send email' type='submit' action={() => { }} fill={true} loading={isLoading} />
